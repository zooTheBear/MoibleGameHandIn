...
class CreditsScene: SKScene {
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        for touch: AnyObject in touches {
            ...
            if backToMainText.contains(pointOfTouch) { // if player touches play game text load game scene
                
                let callBackToMenuScene = MainMenuScene(size: self.size) // make sure game scene is the same size
                callBackToMenuScene.scaleMode = self.scaleMode // same goes for the scale
                let sceneBackToMenuTransition = SKTransition.fade(withDuration: 0.5) // transition to scene in set duration
                self.view!.presentScene(callBackToMenuScene, transition: sceneBackToMenuTransition) // call scene
            }
        }
    }
}
